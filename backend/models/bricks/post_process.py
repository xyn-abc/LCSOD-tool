...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, object_queries, target_sizes):
        ...
        # Extract corresponding object queries
        selected_queries = torch.gather(object_queries, 1,
                                        topk_boxes.unsqueeze(-1).repeat(1, 1, object_queries.shape[-1]))
        ...
        if item_indice is not None:
            ...
            selected_queries = [query[item_index] for query, item_index in zip(selected_queries, item_indice)]

        if torchvision._is_tracing():
            # avoid interation warning during ONNX export
            scores, labels, boxes, queries = map(lambda x: x.unbind(0), (scores, labels, boxes, selected_queries))

        results = [{"scores": s, "labels": l, "boxes": b, "queries": q}
                   for s, l, b, q in zip(scores, labels, boxes, selected_queries)]
