...
def plot_bounding_boxes_on_image_cv2(
    ...
    # Ensure we have enough classes for all labels
    max_label = max(labels) if len(labels) > 0 else 0
    while len(classes) <= max_label:
    ...
    # Safely map labels to class names with bounds checking
    labels = [classes[i] if i < len(classes) else f"class_{i}" for i in labels]
    ...
    for i, (color, label, box) in enumerate(zip(colors, labels, boxes)):
        # get label text - ensure it's a string
        label = str(label)
        if scores is not None:
        ...
        try:
            ...
            # draw text label - ensure label is string
            ...
            cv2.putText(
                image, str(label), (left, top), cv2.FONT_HERSHEY_SIMPLEX, font_scale, font_color, label_size
            )
        except Exception as text_error:
            print(f"Error drawing text '{label}': {text_error}")
...
def plot_bounding_boxes_on_image_pil(
    ...
    try:
        ...
        try:
            # Try alternative font names on Windows
            font = ImageFont.truetype("Arial.ttf", int(15 * font_scale))
        except (OSError, IOError):
            try:
                # Try system font path on Windows
                font = ImageFont.truetype("C:/Windows/Fonts/arial.ttf", int(15 * font_scale))
            except (OSError, IOError):
    ...
    for i, (label, box) in enumerate(zip(labels, boxes)):
        ...
        draw.rectangle([x1, y1 - space + 21, x2, y2 + space + 18], fill=text_box_color)
        font_color = tuple(text_font_color if text_font_color is not None else colors[i])
        draw.text((box[0], box[1] - space + 20), label, fill=font_color, font=font, anchor="lb")
...
def visualize_coco_bounding_boxes(
    ...
    # multiprocess on Windows does not support pickle local functions
